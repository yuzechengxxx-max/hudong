...
import { diagnoseProject, type DiagnosticIssue } from "./core/diagnostics";
...
import { createBrowserProjectRepository } from "./editor/projectRepository";
import { useProjectPersistence } from "./editor/useProjectPersistence";
import { RecoveryPanel } from "./editor/RecoveryPanel";
...
export function App() {
  ...
  const [inspectorHeight, setInspectorHeight] = useState(() => Number(localStorage.getItem("flowfilm-inspector-height")) || 650);
  ...
  const repository = useMemo(() => createBrowserProjectRepository(localStorage), []);
  const persistence = useProjectPersistence(project, repository);
  const saveStatus = persistence.status;
  const savedAt = persistence.lastSaveKind === "manual" ? "刚刚" : persistence.savedAt ? persistence.savedAt.toLocaleTimeString("zh-CN", { hour: "2-digit", minute: "2-digit" }) : "尚未保存";
  ...
  function commitSave() { void persistence.manualSave(); }
  ...
  async function exportWeb() { const { createExportPlayerHtml } = await import("./core/exportPlayer"); download(`${project.title}.html`, createExportPlayerHtml(project), "text/html;charset=utf-8"); }
  ...
  function focusNode(nodeId: string, chapterId: string) { setActiveChapterId(chapterId); setSelectedIds([nodeId]); setFocusRequest({ nodeId, key: Date.now() }); }
  function navigateToVariableReference(nodeId: string, chapterId: string) { focusNode(nodeId, chapterId); }
  function openDiagnosticIssue(issue: DiagnosticIssue) { if (issue.nodeId) { const node = project.nodes.find(item => item.id === issue.nodeId); if (node) focusNode(node.id, node.chapterId); } setShowDiagnostics(false); }
  ...
  async function restoreProject(id: string) { if (!window.confirm("恢复此版本？当前状态会先创建保护点。")) return; const recovered = await persistence.restore(id); setProject(recovered); setActiveChapterId(recovered.defaultChapterId); setSelectedIds([recovered.chapters.find(chapter => chapter.id === recovered.defaultChapterId)?.entryNodeId ?? recovered.nodes[0].id]); pastRef.current = []; futureRef.current = []; }
  ...
  return <EditorShell>
    <header className="topbar tool-island glass-panel" data-testid="tool-island">
      ...
      <button className="toolbar-button" aria-label="保存项目" data-state={saveStatus} onClick={commitSave}>{saveStatus === "saved" ? <CheckCircle2 size={15}/> : <Save size={15}/>} {saveStatus === "saving" ? "保存中" : saveStatus === "saved" ? "已保存" : saveStatus === "error" ? "保存失败" : "保存"}</button>
      ...
      <button className="toolbar-button" aria-label="项目设置" onClick={() => setDrawer(value => value === "settings" ? undefined : "settings")}><Settings size={15}/> 设置</button>
...
function Inspector({ project, selected, updateSelected, onAddChoice, onMoveChoice, onDeleteChoice }: { project: Project; selected: StoryNode; updateSelected(patch: Partial<StoryNode>): void; onAddChoice(): void; onMoveChoice(id: string, direction: -1 | 1): void; onDeleteChoice(id: string): void }) {
  return <>
    ...
    {selected.kind === "scene" && <>
      ...
      {selected.assetId && (() => { const asset = project.assets.find(item => item.id === selected.assetId); return asset ? <div className="inspector-asset-preview">{asset.type.startsWith("image/") ? <img className="contain-media" src={asset.url} alt={asset.name}/> : asset.type.startsWith("video/") ? <video className="contain-media" src={asset.url} aria-label={asset.name} muted/> : <Music/>}<span><b>{asset.name}</b><small>{asset.type}</small></span></div> : null; })()}
...
function FloatingPreview({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => { if (position.x < 70 || position.y < 70) setPosition({ x: 76, y: 76 }); }, [position]);
