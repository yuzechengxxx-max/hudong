...
export function App() {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => commitSave(false), 350);
  ...
  function commitSave(manual = true) { setSaveStatus("saving"); try { const saved = persistProject(localStorage, project); setSavedAt(manual ? "刚刚" : saved.toLocaleTimeString("zh-CN", { hour: "2-digit", minute: "2-digit" })); setSaveStatus("saved"); } catch { setSaveStatus("error"); } }
