...
const StoryNodeView = memo(function StoryNodeView({ data, selected }: NodeProps<Node<GraphData>>) {
  ...
  return <div className={`graph-node ${selected ? "selected" : ""}`} style={{ "--node-color": colors[story.kind], minHeight: Math.max(104, 70 + handles.length * 25) } as React.CSSProperties}>
    ...
    <button className="graph-node-body nodrag" aria-label={story.title} onClick={() => data.onSelect(story.id)}><span>{labels[story.kind]}</span><strong>{story.title}</strong><small>{summary(story)}</small></button>
...
export function StoryGraph(props: { project: Project; selectedId: string; onSelect(id: string): void; onMove(id: string, x: number, y: number): void; onConnect(source: string, port: string, target: string): void; onDeleteNodes(ids: string[]): void; onDeleteEdges(ids: string[]): void }) {
  return <ReactFlowProvider><GraphInner {...props}/></ReactFlowProvider>;
}

function GraphInner({ project, selectedId, onSelect, onMove, onConnect, onDeleteNodes, onDeleteEdges }: Parameters<typeof StoryGraph>[0]) {
  ...
  const edges = useMemo<Edge[]>(() => project.edges.map(edge => ({ id: edge.id, source: edge.source, target: edge.target, sourceHandle: edge.sourcePort, targetHandle: "input", type: "smoothstep", animated: selectedId === edge.source, style: { stroke: selectedId === edge.source ? "#f0b429" : "#77818c", strokeWidth: 2 } })), [project.edges, selectedId]);
  ...
  return <div className="story-graph" data-testid="story-graph">
    <ReactFlow nodes={nodes} edges={edges} nodeTypes={nodeTypes} onNodesChange={onNodesChange} onEdgesChange={onEdgesChange} onConnect={handleConnect} fitView minZoom={0.12} maxZoom={2.5} snapToGrid snapGrid={[16,16]} panOnScroll selectionOnDrag connectionMode={ConnectionMode.Loose} deleteKeyCode={["Backspace","Delete"]} multiSelectionKeyCode="Shift" attributionPosition="bottom-left">
      <Background gap={20} size={1}/><MiniMap pannable zoomable nodeColor={node => colors[(node.data as GraphData).story.kind]}/><Controls showInteractive={false}/>
