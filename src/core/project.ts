...
export const StoryNodeSchema = z.discriminatedUnion("kind", [
  ...
  BaseNodeSchema.extend({ kind: z.literal("chapter"), chapterId: z.string().min(1) }),
  BaseNodeSchema.extend({ kind: z.literal("jump"), chapterId: z.string().min(1) }),
...
export function createNode(kind: Exclude<NodeKind, "start">, index: number): StoryNode {
  ...
  if (kind === "chapter") return { ...base, kind, title: "章节入口", chapterId: `chapter-${index + 1}` };
  if (kind === "jump") return { ...base, kind, title: "跳转章节", chapterId: "chapter-1" };
