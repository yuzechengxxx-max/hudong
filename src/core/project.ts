...
export const StoryNodeSchema = z.discriminatedUnion("kind", [
  ...
  BaseNodeSchema.extend({ kind: z.literal("scene"), assetId: z.string().optional(), mediaUrl: z.string(), speaker: z.string(), dialogue: z.string() }),
  BaseNodeSchema.extend({ kind: z.literal("choice"), prompt: z.string(), choices: z.array(z.object({ id: z.string(), label: z.string() })).min(2) }),
...
export function createStarterProject(): Project {
  return {
    ...
    nodes: [
      ...
      { id: "opening", kind: "scene", title: "雨夜码头", position: { x: 275, y: 110 }, mediaUrl: "", speaker: "林夏", dialogue: "那封信，真的来自他吗？" },
...
export function createNode(kind: Exclude<NodeKind, "start">, index: number): StoryNode {
  ...
  if (kind === "scene") return { ...base, kind, title: "新场景", mediaUrl: "", speaker: "角色", dialogue: "输入对白内容" };
